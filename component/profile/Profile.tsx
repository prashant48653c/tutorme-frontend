'use client'
import { Button } from "@/components/ui/button";
import { Inbox } from "lucide-react";
import Image from "next/image";
import { useState } from "react";
import EditProfileForm from "./EditProfile";
 


const Profile = () => {
    const progress = 60; // progress percentage (0 to 100)
    const [status, setStatus] = useState(false);
  return (
     <div className="shadow-md   rounded-2xl p-4 pb-7 ">
            <div className="text-center flex flex-col items-center my-3">
              <div className="relative w-32 h-32">
                <div
                  className="absolute  inset-0 rounded-full"
                  style={{
                    background: `conic-gradient(from 25deg, #09C4AE ${progress}%, transparent ${progress}% 100%)`,
                    transform: "scaleX(-1)",
                  }}
                ></div>

                <div className="absolute inset-[6px] rounded-full overflow-hidden bg-white">
                  <Image
                    src="/static/landing/course.svg"
                    alt="profile"
                    width={96}
                    height={96}
                    className="w-full h-full p-1 rounded-full
                   object-cover"
                  />
                </div>
              </div>

              <p className="my-2 text-green-500 text-sm">
                Upload Profile Picture
              </p>
            </div>

            <div>
               {status && (
                      <div className="absolute z-10 left-1/2 top-40 transform -translate-x-1/2 p-2 text-xs text-gray-700">
                  <EditProfileForm />
                </div>
                    )}
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    <h5 className="text-2xl font-semibold">Sandesh Sapkota</h5>
                    <Image
                      src={"/static/icons/verified.svg"}
                      width={30}
                      height={30}
                      alt="profile"
                      className="rounded-full object-cover w-6 h-6"
                    />
                  </div>
                  <p className="text-lg text-green-500">UI/UX Designer</p>
                </div>

                <div>
                  <Button onClick={()=>setStatus(true)} className="bg-gray-100 hover:bg-green-100 text-green-500 font-normal">
                    Edit Profile
                  </Button>
                </div>
              </div>
              <div className="grid mt-5 grid-cols-2 grid-rows-3 gap-y-3">
                <div className="flex items-center justify-start gap-2">
                  <Image
                    src="/static/icons/d1.svg"
                    alt="location"
                    width={20}
                    height={20}
                  />

                  <p>sandesh@gmail.com</p>
                </div>

                <div className="flex items-center justify-start gap-2">
                  <Image
                    src="/static/icons/d2.svg"
                    alt="location"
                    width={20}
                    height={20}
                  />
                  <p>Baneshwor</p>
                </div>
                <div className="flex items-center justify-start gap-2">
                  <Image
                    src="/static/icons/d4.svg"
                    alt="location"
                    width={20}
                    height={20}
                  />

                  <p>4 Courses</p>
                </div>
                <div className="flex items-center justify-start gap-2">
                  <Image
                    src="/static/icons/d3.svg"
                    alt="location"
                    width={20}
                    height={20}
                  />

                  <p>Technergy Global Pvt. Ltd.</p>
                </div>
                <div className="flex items-center justify-start gap-2">
                  <Image
                    src="/static/icons/d5.svg"
                    alt="location"
                    width={20}
                    height={20}
                  />

                  <p>93839393839</p>
                </div>

                <div className="flex items-center justify-start gap-2">
                  <Image
                    src="/static/icons/d6.svg"
                    alt="location"
                    width={20}
                    height={20}
                  />

                  <p>English, Nepali</p>
                </div>
              </div>
            </div>
          </div>
  )
}

export default Profile