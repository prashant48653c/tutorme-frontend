...
function FilterSection({ title, data, showClearAll, onItemChange, onClearAll }: FilterSectionProps) {
  return (
    <SidebarGroup>
      <div className="flex items-center justify-between">
        <SidebarGroupLabel className="text-teal-600  font-medium text-md mb-1">{title}</SidebarGroupLabel>
        {showClearAll && (
          <Button
            variant="link"
            size="sm"
      ...
      <SidebarGroupContent className="space-y-2">
        {data.map((item) => (
          <div key={item.id} className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor={item.id} className="flex-1 text-sm font-normal cursor-pointer">
              {item.label}
            </Label>
            {item.count !== undefined && <span className="text-xs text-gray-500 ml-auto">{item.count}</span>}
          </div>
        ))}
      </SidebarGroupContent>
    </SidebarGroup>
...
export function CourseFilterSidebar() {
  ...
  const [durationFilters, setDurationFilters] = useState<FilterItem[]>([
    { id: "less-than-2hrs", label: "Less than 2 hrs", count: 387, checked: false },
  ...
  useEffect(() => {
    ...
    setSemesterFilters(prev => prev.map(item => {
    ...
    setLevelFilters(prev => prev.map(item => ({
      ...item,
      checked: courseDepth.includes(item.label.toLowerCase())
    })));
    
    setDurationFilters(prev => prev.map(item => {
  ...
  const handleClearAllFilters = () => {
    ...
    setSemesterFilters(prev => prev.map(item => ({ ...item, checked: false })));
    setLanguageFilters(prev => prev.map(item => ({ ...item, checked: false })));
    setLevelFilters(prev => prev.map(item => ({ ...item, checked: false })));
    setDurationFilters(prev => prev.map(item => ({ ...item, checked: false })));
  };
   ...
   <div className="w-64 ">
      <div className="">
        <Button
          variant="outline"
          size="sm"
          className="justify-start border border-green-400 px-5   gap-2 bg-transparent"
